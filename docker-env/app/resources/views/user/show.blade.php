      ...
      @can('general')
        @if($users[0]['id']!=Auth::user()->id)<!-- フォロー機能 -->
          @if(in_array($users[0]['id'],$myfollow))
            <a href="{{ route('remove_follow',['user'=>$users[0]['id']])}}" class="nav-link link-dark btn btn-outline-danger btn-sm">フォロー解除</a>
          @else
            <a href="{{ route('add_follow',['user'=>$users[0]['id']])}}" class="nav-link link-dark btn btn-outline-primary btn-sm">&emsp;フォロー&emsp;</a>
          @endif
        @endif
      @endcan
